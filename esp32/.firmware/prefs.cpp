...
void prefs_load(FSDState *state) {
    ...
    state->tsllc_stops              = g_prefs.getBool("tsllc_s",false);
    state->continue_on_green        = g_prefs.getBool("cogcipv",false);
    state->disable_telemetry        = g_prefs.getBool("notelem",false);
...
void prefs_save(const FSDState *state) {
    ...
    g_prefs.putBool("tsllc_s",state->tsllc_stops);
    g_prefs.putBool("cogcipv",state->continue_on_green);
    g_prefs.putBool("notelem",state->disable_telemetry);
